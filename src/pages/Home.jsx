				...
				<div className="container">
					...
					<div className="row mt-2">
						<div className="col-md mb-2">
							<img className="w-100" src={process.env.REACT_APP_IMG_URL + "slides/" + banners[0]?.desktop} alt="" />
						</div>
					</div>
						...
						<ul className="list-inline text-center" style={{ fontSize: "20px" }}>
							...
							{collections &&
								collections.map((collection, index) => (
									<li style={{ cursor: "pointer" }} onClick={(e) => setIdCollection(collection.id)} className="list-inline-item ps-4">
										{collection.name}
									</li>
								))}
						</ul>
								...
								<div className="col-md-4 mb-3">
									<div class="card shadow border-0">
										...
										<div class="card-body border border-1">
											<Link style={{ textDecoration: "none" }} to={"/dich-vu/" + service.slug}>
												{" "}
												<h4 class="card-title text-center">{service.name}</h4>
											</Link>
											<h4 class="card-title text-center">
												<span className="text-danger">{Intl.NumberFormat("en-US").format(service.price)}</span>{" "}
												<span className="text-decoration-line-through">{Intl.NumberFormat("en-US").format(service.compare_price)}</span>
											</h4>
										</div>
									</div>
								</div>
