                            ...
                            <tbody>
                              {carts.map((item, index) => (
                                <tr key={index}>
                                  <td scope="row">
                                    <a href={`/${item.slug}`}>
                                      <img src={`${process.env.REACT_APP_IMG_URL}products/${item.image}`} width={150} alt="" />
                                    </a>
                                  </td>
                                  <td>
                                    ...
                                    <div className="product-price d-flex align-items-center gap-2 mt-3">
                                      <div className="h6 fw-bold">
                                        <span className="text-decoration-line-through pe-2">{Intl.NumberFormat("en-US").format(Number(item.price))}</span>
                                        <span className="text-danger">{Intl.NumberFormat("en-US").format(Number(item.discount))}</span>
                                        <br />
                                        <label htmlFor="" className="mt-2">
                                          Số lượng
                                        </label>
                                        <div className="input-group mb-3">
                                          <input type="number" className="form-control mt-2" value={item.quantity} onChange={(e) => updateQuantity(item.id, e)} />
                                          <button className="btn btn-outline-danger mt-2" type="button" onClick={() => deleteItem(item.id)}>
                                            Xoá
                                          </button>
                                        </div>
                                      </div>
                                  ...
                                  <td>{Intl.NumberFormat("en-US").format(Number(item.total))}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                  ...
                  {carts.length > 0 && (
                    <>
                      <div className="hstack align-items-center justify-content-between">
                        ...
                        <input type="text" className="form-control" placeholder="Tên người mua" onChange={(e) => setFullname(e.target.value)} />
                      </div>
                      <hr />
                      <div className="hstack align-items-center justify-content-between">
                      ...
                      <div className="hstack align-items-center justify-content-between">
                        ...
                        <br />
                        <input type="text" placeholder="Số điện thoại" onChange={(e) => setPhone(e.target.value)} className="form-control" />
                      </div>
                      <hr />
                      <div className="hstack align-items-center justify-content-between">
                        <p className="mb-0">Địa chỉ</p>
                        <input type="text" placeholder="Địa chỉ nhận hàng" onChange={(e) => setAddress(e.target.value)} className="form-control" />
                      </div>
                      <div className="d-grid mt-4">
                        <button onClick={(e) => submitBook()} type="button" className="btn btn-dark btn-ecomm py-3 px-5">
                          Place Order
                        </button>
                      </div>
                    </>
